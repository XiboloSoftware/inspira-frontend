...
export default function SolicitudDetalleBackoffice({ idSolicitud, onVolver }) {
  ...
  async function cargar() {
    ...
    setError("");
    try {
      // Usa la ruta que añadimos en documentos.routes:
      // GET /api/admin/solicitudes/:idSolicitud/checklist
      const r = await boGET(`/admin/solicitudes/${idSolicitud}/checklist`);
      if (!r.ok) {
        setError(
  ...
  async function cambiarRevision(doc, nuevoEstado) {
    ...
    try {
      const r = await boPATCH(
        `/admin/documentos/${doc.id_documento}/revision`,
        {
      ...
      setChecklist((prev) =>
        prev.map((it) => ({
          ...it,
          documentos: (it.documentos || []).map((d) =>
            d.id_documento === actualizado.id_documento ? actualizado : d
          ),
        }))
      );
    } catch (e) {
                    ...
                    <div className="mt-1 space-y-1">
                      ...
                      {(it.documentos || []).map((doc) => (
                        <div
                          ...
                          className="flex items-center justify-between gap-2 border-t border-neutral-100 pt-1 mt-1"
                        >
                          ...
                          <div className="flex items-center gap-1">
                            {/* IMPORTANTE: backend debería devolver url_descarga */}
                            {doc.url_descarga && (
                              <a
